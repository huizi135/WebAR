...
/** Contains UI elements for Hello EIS. */
class HelloEisView(val activity: HelloEisActivity) : DefaultLifecycleObserver {
        ...
        PopupMenu(activity, v).apply {
          setOnMenuItemClickListener { item ->
            when (item.itemId) {
              ...
              R.id.eis_settings -> launchEisSettingsMenuDialog()
              else -> null
            } != null
          }
  ...
  /** Shows checkbox to the user for controlling EIS effect. */
  private fun launchEisSettingsMenuDialog() {
    val session = session ?: return
    val resources = activity.resources
    val strings = resources.getStringArray(R.array.eis_options_array)
    val checked = booleanArrayOf(activity.eisSettings.isEisEnabled)
    if (session.isImageStabilizationModeSupported(Config.ImageStabilizationMode.EIS)) {
      AlertDialog.Builder(activity)
        .setTitle(R.string.options_title_eis)
        .setMultiChoiceItems(strings, checked) { _, which, isChecked -> checked[which] = isChecked }
        .setPositiveButton(R.string.done) { _, _ ->
          activity.eisSettings.isEisEnabled = checked[0]
          activity.configureSession(session)
        }
        .show()
    } else {
      // Without EIS support, no settings are available.
      AlertDialog.Builder(activity)
        .setTitle("Your device does not support image stabilization.")
        .setPositiveButton("Done") { _, _ -> /* No settings to apply. */ }
        .show()
    }
  }
}
